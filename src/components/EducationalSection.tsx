...
export const EducationalSection = ({ ammType, lastSwapResult, swapDirection, tokenA, tokenB }: EducationalSectionProps) => {
  ...
  return (
    <section className="bg-white text-slate-900 p-8 md:p-12 rounded-3xl border border-slate-200 shadow-sm space-y-12">
      ...
      <AnimatePresence mode="wait">
        {lastSwapResult && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="grid grid-cols-1 md:grid-cols-4 gap-6 bg-slate-50 p-8 rounded-2xl border border-slate-100 backdrop-blur-sm"
          >
            <div className="space-y-2">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Realized Output</div>
              <div className="text-xl font-bold text-slate-900 flex items-center truncate">
            ...
            <div className="space-y-2">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Price Impact</div>
              <div className={cn("text-xl font-bold", lastSwapResult.priceImpact > 5 ? "text-rose-600" : "text-emerald-600")}>
                {lastSwapResult.priceImpact.toFixed(2)}%
              </div>
              <p className="text-[10px] text-slate-500 leading-tight">Difference between market price and realized price.</p>
            </div>

            <div className="space-y-2">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">LP Fee (0.3%)</div>
              <div className="text-xl font-bold text-amber-600">
                {lastSwapResult.fee.toFixed(4)} {inToken.symbol}
              </div>
              <p className="text-[10px] text-slate-500 leading-tight">100% enters pool; fee stays as LP reward.</p>
            </div>

            <div className="space-y-2">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Invariant Shift ({lastSwapResult.invariantName})</div>
              <div className="text-xl font-bold text-sky-600 flex items-center gap-2">
              ...
              <p className="text-[10px] text-emerald-600 font-medium">↑ Improved via fee accumulation</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 bg-slate-50 p-8 rounded-2xl border border-slate-100">
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-indigo-600 font-bold uppercase text-xs tracking-[0.2em]">
            Key Advantage
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-rose-600 font-bold uppercase text-xs tracking-[0.2em]">
            Main Tradeoff
          </div>
          <p className="text-slate-600 text-lg leading-relaxed">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl hover:bg-slate-100 transition-colors group">
